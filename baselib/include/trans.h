...
public:
    ...
    */
    static int STOI(const char* szValue);

    /*!
     * \brief 解析并把字符串转化为整型数据
    */
    static int STOI(const std::string& sValue);
    ...
    */
    static const std::string ITOS(int iValue, std::streamsize nPrecision=-1, std::streamsize nWide=-1, char chFill=' ', std::ios::fmtflags fmt=std::ios::fixed);

    /*!
     * \brief 把整型数据转化为字符串
     * \param[in] uValue: 待转换的数据
     * \param[in] nPrecision: 小数精度位数(在些无效)
     * \param[in] nWidth: 总位数
     * \param[in] nFill: 填充字符
     * \param[in] fmt: 其它IOS流标志,详细标志可参考IOS标志
     * \return string 转换好的字符串
    */
    static const std::string UTOS(unsigned uValue, std::streamsize nPrecision=-1, std::streamsize nWide=-1, char chFill=' ', std::ios::fmtflags fmt=std::ios::fixed);
...
inline std::string operator + (const std::string& _s, const int& _i)
{
	return _s+CTrans::ITOS(_i);
}

inline std::string operator + (const int& _i, const std::string& _s)
{
